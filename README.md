...
| Encode | Media Foundation `IMFSinkWriter` → H.264 + AAC / MP4. The encoder is tuned for quality-per-byte: **High profile** (CABAC + 8×8 transform), **B-frames**, and a quality-leaning speed setting, configured via the encoder's `ICodecAPI` (`H264EncoderConfig`) instead of the Baseline default |
